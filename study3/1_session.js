...
// npm install express-session -> npm i express-session  이라고 써도됨
...
app.get('/login',(req,res)=>{
    ...
    if(!req.session.member){
        ...
        res.write(`<p><a href='/main'>메인으로 이동</p>`);
        res.end();
    }else{
        console.log('이미 로그인 중입니다.');
        res.redirect('/main'); //이미 로그인중이니 메인으로 다시 보내버림
    }
});
...
app.get('/main',(req,res)=>{
    if(req.session.member){
        ...
        fs.readFile('welcome.html','utf8',(err,data)=>{
            res.writeHead(200,{'content-type':'text/html;charset=utf-8;s'})
            res.end(data);
        });
    }else{
